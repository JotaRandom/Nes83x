...
impl MMC3 {
    /// Create a new MMC3 mapper from a cartridge
...
impl Mapper for MMC3 {
    ...
    fn reset(&mut self) {
        ...
        // Set initial bank values
        self.r[6] = 0xFE; // Last bank - 1 at 0x8000-0x9FFF
        self.r[7] = 0xFF; // Last bank at 0xA000-0xBFFF
    }
...
mod tests {
    ...
    fn test_mmc3_initial_state() {
        ...
        assert_eq!(mmc3.r[6], 0xFE); // Last bank - 1 at 0x8000-0x9FFF
        assert_eq!(mmc3.r[7], 0xFF); // Last bank at 0xA000-0xBFFF
    }
    
    #[test]
    fn test_mmc3_prg_banking() {
        let cartridge = create_test_cartridge();
        let mut mmc3 = MMC3::new(&cartridge);
    ...
    fn test_mmc3_irq() {
        ...
        // Disable IRQ
        mmc3.write_prg_byte(0xE000, 0x00); // Disable IRQ
        assert!(!mmc3.irq_enabled);
        assert!(!mmc3.irq_occurred);
    }
}
