use anyhow::Result;
use clap::Parser;
use log::{error, info};
use simplelog::{ColorChoice, Config, LevelFilter, TermLogger, TerminalMode};
use std::path::PathBuf;

use nes83x::emulator::gtk::Emulator;

/// Command-line arguments
#[derive(Parser, Debug)]
#[command(version, about, long_about = None)]
struct Args {
    /// ROM file to load (optional - GUI will be shown if not provided)
    rom: Option<PathBuf>,

    /// Directory containing ROMs (default: roms)
    #[arg(short, long, default_value = "roms")]
    rom_dir: PathBuf,

    /// Enable debug logging
    #[arg(short, long)]
    debug: bool,

    /// Scale factor for the window (default: 2)
    #[arg(short, long, default_value_t = 2)]
    scale: u32,
}

fn main() -> Result<()> {
    // Parse command-line arguments
    let args = Args::parse();

    // Initialize logging
    let log_level = if args.debug {
        LevelFilter::Debug
    } else {
        LevelFilter::Info
    };

    TermLogger::init(
        log_level,
        Config::default(),
        TerminalMode::Stderr,
        ColorChoice::Auto,
    )?;

    info!("Starting NES83x GTK emulator");

    // Try to detect and set appropriate backend
    eprintln!("WAYLAND_DISPLAY: {:?}", std::env::var("WAYLAND_DISPLAY"));
    eprintln!("DISPLAY: {:?}", std::env::var("DISPLAY"));
    if std::env::var("WAYLAND_DISPLAY").is_ok() {
        std::env::set_var("GDK_BACKEND", "wayland");
        eprintln!("Set GDK_BACKEND to wayland");
    } else if std::env::var("DISPLAY").is_ok() {
        std::env::set_var("GDK_BACKEND", "x11");
        eprintln!("Set GDK_BACKEND to x11");
    } else {
        eprintln!("No display detected, GTK will likely fail");
    }
    eprintln!("GDK_BACKEND: {:?}", std::env::var("GDK_BACKEND"));

    // Create the GTK emulator frontend
    let mut emulator = match Emulator::new() {
        Ok(emulator) => emulator,
        Err(e) => {
            error!("Failed to create GTK emulator: {}", e);
            error!("Make sure you have a display server running (X11) and DISPLAY is set correctly");
            error!("For WSL: Install VcXsrv or X410 on Windows and set DISPLAY=:0");
            error!("For native Linux: Make sure X11 is running");
            return Err(anyhow::anyhow!("GTK initialization failed: {}", e));
        }
    };

    // Initialize the UI
    if let Err(e) = emulator.init() {
        error!("Failed to initialize emulator UI: {}", e);
        return Err(anyhow::anyhow!("UI initialization failed: {}", e));
    }

    // If a ROM was specified via command line, load it
    if let Some(rom_path) = args.rom {
        info!("Loading ROM from command line: {}", rom_path.display());
        if let Err(e) = emulator.load_rom(rom_path) {
            error!("Failed to load ROM: {}", e);
            return Err(anyhow::anyhow!("ROM loading failed: {}", e));
        }
    }
    // Note: Auto-loading from roms/ directory disabled to start without ROM for debugging

    if emulator.rom_path.is_some() {
        info!("ROM loaded: {:?}", emulator.rom_path);
    } else {
        info!("No ROM loaded, starting with placeholder");
    }

    // Run the emulator
    if let Err(e) = emulator.run() {
        error!("Emulator error: {}", e);
        return Err(anyhow::anyhow!("Emulator runtime error: {}", e));
    }

    Ok(())
}